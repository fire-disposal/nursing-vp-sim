...
class SimulationSession(Base, TimestampMixin):
    ...
    The outcome status has a single owner: ``state.case_status`` (vocabulary in
    ``core.statuses.SimulationStatus``). There is deliberately no SQL ``status``
    column — a second copy cannot be constrained against the JSONB truth and
    only invited write-only drift (见 f4a5b6c7d8e9 迁移)。"""

    __tablename__ = "simulation_sessions"
    __table_args__ = (Index("ix_simulation_sessions_user", "user_id"),)
