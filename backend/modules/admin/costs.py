"""Admin cost dashboard — router + service."""
...
class CostService:
    ...
    def get_dashboard(self) -> CostDashboardResponse:
        ...
        today_latency = (
            ((float(llm_today[3]) * llm_today[0] + float(voice_today[3]) * voice_today[0]) / today_total)
            if today_total > 0
            else 0.0
        )
        ...
        month_latency = (
            ((float(llm_month[3]) * llm_month[0] + float(voice_month[3]) * voice_month[0]) / month_total)
            if month_total > 0
            else 0.0
        )
