...
被写入时的内容指纹」（``content_hash`` 对去掉指纹键后的 payload 做 sha256）。用它与
「当前内容的指纹」比对，才能区分三种行：
...
def content_hash(case_data: dict[str, Any] | None) -> str:
    ...
    """
    payload = {k: v for k, v in (case_data or {}).items() if k != SEED_HASH_KEY}
    blob = json.dumps(payload, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(blob.encode("utf-8")).hexdigest()


def with_seed_bookmark(file_data: dict[str, Any]) -> dict[str, Any]:
    """落库 payload：仓库病例内容 + 其内容指纹。"""
    return {**file_data, SEED_HASH_KEY: content_hash(file_data)}
