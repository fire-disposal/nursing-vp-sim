...
from .prompts import DIAGNOSIS_REVIEW_SYSTEM, EXPERT_CONSULT_SYSTEM
from .service import ConsultProvider, DiagnoseProvider, SimulationService, TalkProvider, build_snapshot
...
def _talk_provider(request: Request, user_id: int) -> TalkProvider | None:
    ...
    The persona system prompt is supplied by the service (built from the
    case's patient/family background) — the router only owns the LLM call.
    """
    llm_client = getattr(request.app.state, "llm_client", None)
    if llm_client is None:
        return None

    def provider(system: str, summary: str, line: str) -> str:
        async def _run() -> str:
            return await llm_client.call(
                [
...
def _diagnose_provider(request: Request, user_id: int) -> DiagnoseProvider | None:
    """Diagnosis-review provider: scores the player's diagnosis at case end."""
    llm_client = getattr(request.app.state, "llm_client", None)
    if llm_client is None:
        return None

    def provider(review_prompt: str) -> str:
        async def _run() -> str:
            return await llm_client.call(
                [
                    {"role": "system", "content": DIAGNOSIS_REVIEW_SYSTEM},
                    {"role": "user", "content": review_prompt},
                ],
                purpose="diagnosis_review",
                ctx=CallContext(purpose="diagnosis_review", user_id=user_id),
            )
