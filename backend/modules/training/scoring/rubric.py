...
_NURSING_RECORD_DIMENSION = {
    ...
    "max": 10,
    "description": (
        "评估已提交（冻结）的护理评估记录（ADPIE）的完整性与合理性，"
        "以及结构化护理诊断（护理问题/相关因素/定义特征）的专业性与优先级排序"
    ),
    "items": [
        {
            "id": "nr_01",
            "name": "护理评估记录的完整性和结构性",
            "anchors": {
                "2": "五步齐全(主观/客观/评估/计划/评价)，每个部分内容充实",
                "1": "多数步骤已填写但部分缺失或内容过简",
                "0": "未填写护理记录或内容严重缺失",
            },
        },
        {
            ...
            "name": "护理评估的临床合理性与专业性",
            "anchors": {
                "2": "评估具有逻辑性，护理诊断与采集的病史数据一致，方案具有针对性",
                "1": "评估基本合理但部分环节存在逻辑偏差或方案泛化",
                "0": "评估缺乏逻辑性或严重脱离病史采集内容",
            },
        },
        {
            "id": "nr_03",
            ...
            "anchors": {
                "2": "主客观数据准确反映对话中的患者陈述和查体结果，无编造",
                "1": "大部分数据来自对话，但存在少量不准确或推断性表述",
                "0": "护理记录存在大量编造或与对话内容明显矛盾的数据",
            },
        },
        {
            "id": "nr_04",
            ...
            "anchors": {
                "2": "结构化护理诊断（问题+相关因素+定义特征）明确，主要护理问题按优先级排序，措施具体可行",
                "1": "护理问题识别基本正确但诊断三要素不全、排序不够合理，措施过于泛化",
                "0": "未识别关键护理问题或措施与问题不匹配",
            },
        },
        {
            "id": "nr_05",
            "name": "评价环节的反思深度",
            "anchors": {
                "2": "评价体现对措施效果的批判性反思，提出后续跟进建议",
                "1": "有简单的效果评价但缺乏深入反思",
                "0": "无评价环节或评价填空性质不反映实际思考",
            },
        },
    ],
}
...
def build_final_rubric(base_rubric: dict, features: dict | None = None) -> dict:
    ...
    if features and features.get("nursing_record"):
        ...
        if "nursing_record" not in existing_ids:
            ...
            rubric["raw_max"] = rubric.get("raw_max", 38) + 10  # 5 条目 × raw_scale 2
    return rubric
