...
def list_secrets(current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    for s in secrets:
        ...
        result.append(ApiSecretResponse(
            ...
            status=s.status, degraded_reason=s.degraded_reason, degraded_until=s.degraded_until,
            ...
            config_count=config_count, last_used_at=s.last_used_at,
...
async def create_secret(data: ApiSecretCreate, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    suffix = data.raw_key[-4:] if len(data.raw_key) >= 4 else "****"
    s = ApiSecret(
        label=data.label, encrypted_key=encrypt_api_key(data.raw_key),
        key_suffix=suffix, base_url=data.base_url or "",
...
def update_secret(secret_id: int, data: ApiSecretUpdate, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    s = db.query(ApiSecret).filter(ApiSecret.id == secret_id).first()
    if not s: raise HTTPException(404, "档案不存在")
    for field in ("label", "base_url", "price_input_per_1m", "price_output_per_1m", "monthly_cost_limit"):
        val = getattr(data, field, None)
        if val is not None: setattr(s, field, val)
    db.commit()
    return {"ok": True}


@router.delete("/secrets/{secret_id}", response_model=OkResponse)
async def delete_secret(secret_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    s = db.query(ApiSecret).filter(ApiSecret.id == secret_id).first()
    if not s: raise HTTPException(404, "档案不存在")
    count = db.query(LLMConfig).filter(LLMConfig.secret_id == secret_id).count()
    if count > 0: raise HTTPException(400, f"该档案有 {count} 个用途绑定，先解除")
...
def list_configs(purpose: str | None = Query(None), current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    q = db.query(LLMConfig)
    if purpose: q = q.filter(LLMConfig.purpose == purpose)
    ...
    for c in configs:
        ...
        result.append(LLMConfigResponse(
            ...
            model=c.model, purpose=c.purpose, status=c.status,
...
async def create_config(data: LLMConfigCreate, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    secret = db.query(ApiSecret).filter(ApiSecret.id == data.secret_id).first()
    if not secret: raise HTTPException(404, "档案不存在")
    ...
    if existing:
        ...
        db.commit(); await refresh_router()
...
async def update_config(config_id: int, data: LLMConfigUpdate, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    cfg = db.query(LLMConfig).filter(LLMConfig.id == config_id).first()
    if not cfg: raise HTTPException(404, "指派不存在")
    for f in ("model", "purpose", "status"):
        val = getattr(data, f, None)
        if val is not None: setattr(cfg, f, val)
    db.commit(); await refresh_router()
    return {"ok": True}


@router.delete("/configs/{config_id}", response_model=OkResponse)
async def delete_config(config_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    cfg = db.query(LLMConfig).filter(LLMConfig.id == config_id).first()
    if not cfg: raise HTTPException(404, "指派不存在")
    db.delete(cfg); db.commit(); await refresh_router()
    return {"ok": True}


@router.post("/configs/{config_id}/toggle", response_model=ToggleStatusResponse)
async def toggle_config(config_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    cfg = db.query(LLMConfig).filter(LLMConfig.id == config_id).first()
    if not cfg: raise HTTPException(404, "指派不存在")
...
async def reset_profile(config_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    cfg = db.query(LLMConfig).filter(LLMConfig.id == config_id).first()
    if not cfg: raise HTTPException(404, "指派不存在")
...
async def test_config(config_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    cfg = db.query(LLMConfig).filter(LLMConfig.id == config_id).first()
    if not cfg: raise HTTPException(404, "指派不存在")
    secret = db.query(ApiSecret).filter(ApiSecret.id == cfg.secret_id).first()
...
async def test_all_configs(current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(8)) as client:
        for s in secrets:
            ...
            try:
                ...
                resp = await client.get(f"{base_url}/v1/models", headers={"Authorization": f"Bearer {api_key}"})
                latency = int((time.monotonic() - t0) * 1000)
                results.append({"base_url": base_url, "ok": resp.status_code < 500, "status_code": resp.status_code, "latency_ms": latency})
            except Exception as e:
...
async def reload_router(current_user: User = Depends(require_teacher)):
    await refresh_router(); return {"ok": True}


@router.get("/model-presets", response_model=CatalogResponse)
def list_model_presets(current_user: User = Depends(require_teacher)):
    catalog = get_catalog()
    providers = []
    for p in catalog["providers"]:
        models = [ModelPresetItem(name=m["name"], price_input=m.get("price_input", 0), price_output=m.get("price_output", 0)) for m in p.get("models", [])]
        providers.append(ProviderPresetResponse(provider=p["id"], display_name=p.get("display_name", p["id"]), base_url=p.get("base_url", ""), models=models))
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(5)) as client:
        for s in secrets:
            ...
            try:
                ...
                latency = int((time.monotonic() - t0) * 1000)
                results.append({"base_url": s.base_url, "status": "ok" if resp.status_code < 500 else "error", "latency_ms": latency, "error": None})
            except Exception as e:
...
@router.get("/fallback")
def get_env_fallback(current_user: User = Depends(require_teacher)):
...
async def test_env_fallback(current_user: User = Depends(require_teacher)):
    from config import DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL
    if not DEEPSEEK_API_KEY: return {"base_url": DEEPSEEK_BASE_URL, "ok": False, "error": "DEEPSEEK_API_KEY 未设置"}
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(10)) as client:
            t0 = time.monotonic()
            resp = await client.get(f"{DEEPSEEK_BASE_URL}/v1/models", headers={"Authorization": f"Bearer {DEEPSEEK_API_KEY}"})
            return {"base_url": DEEPSEEK_BASE_URL, "ok": resp.status_code < 400, "status_code": resp.status_code, "latency_ms": int((time.monotonic() - t0) * 1000)}
    except Exception as e:
...
def get_active_rubric(current_user: User = Depends(require_teacher)):
    ...
    if not active: raise HTTPException(404, "没有激活的评分标准")
    return active
...
def create_rubric(data: dict, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    if not dims: raise HTTPException(400, "dimensions 不能为空")
    errors = validate_dimensions(dims)
    if errors: raise HTTPException(400, "; ".join(errors))
    rubric = Rubric(name=data.get("name", ""), version=data.get("version", "1.0"), description=data.get("description"),
                    total_max=data.get("total_max", 100), raw_max=data.get("raw_max", 57), raw_scale=data.get("raw_scale", 3), dimensions=dims)
...
def update_rubric(rubric_id: int, data: dict, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    if not rubric: raise HTTPException(404, "评分标准不存在")
    ...
    for field in ("name", "version", "description", "total_max", "raw_max", "raw_scale"):
        if field in data: setattr(rubric, field, data[field])
    db.commit(); return rubric
...
def delete_rubric(rubric_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    rubric = db.query(Rubric).filter(Rubric.id == rubric_id).first()
    if not rubric: raise HTTPException(404, "评分标准不存在")
    if rubric.is_active: raise HTTPException(400, "不能删除当前激活的评分标准")
    db.delete(rubric); db.commit(); return {"ok": True}
...
def activate_rubric(rubric_id: int, current_user: User = Depends(require_teacher), db: Session = Depends(get_db)):
    ...
    rubric.is_active = True; db.commit(); return {"ok": True}
