...
def test_sweep_stale_scoring_records_marks_old(db_session):
    ...
    from infrastructure.settlement import STALE_SCORING_SWEEP_MINUTES, _sweep_stale_scoring_records
...
def test_sweep_stale_scoring_records_handles_null_end_time(db_session):
    ...
    from infrastructure.settlement import _sweep_stale_scoring_records
...
def test_end_training_queue_full_rollback(client, db_session, monkeypatch):
    ...
    user = User(
        username="qfull_end_user",
        password_hash=hash_password("test123"),
        role_id=student_role.id,
        display_name="QF End User",
    )
    db_session.add(user)
    db_session.flush()
    ...
    for i in range(3):
        msg = Message(record_id=rec.id, role="student", content="A" * 100 + f" msg {i}")
        db_session.add(msg)
    db_session.commit()

    resp = client.post("/api/auth/login", json={"username": "qfull_end_user", "password": "test123"})
    assert resp.status_code == 200, resp.text
    token = resp.json()["access_token"]

    async def _enqueue_raise(*args, **kwargs):
        raise QueueFullError("queue full")

    monkeypatch.setattr(client.app.state.task_queue, "enqueue", _enqueue_raise)
    ...
    db_session.expire_all()
    updated = db_session.query(TrainingRecord).filter(TrainingRecord.id == rec.id).first()
    assert updated.scoring_status is None, f"expected NULL, got {updated.scoring_status}"
...
def test_retry_scoring_queue_full_preserves_old_score(client, db_session, monkeypatch):
    ...
    teacher = User(
        username="qfull_retry_user",
        password_hash=hash_password("test123"),
        role_id=1,
        display_name="QF Retry Teacher",
    )
    ...
    for i in range(3):
        msg = Message(record_id=rec.id, role="student", content="B" * 100 + f" msg {i}")
        db_session.add(msg)
    db_session.commit()

    old_score_id = score.id

    resp = client.post("/api/auth/login", json={"username": "qfull_retry_user", "password": "test123"})
    assert resp.status_code == 200, resp.text
    token = resp.json()["access_token"]
    ...
    resp = client.post(
        f"/api/training/{rec.id}/retry-scoring",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 503
    ...
    score_count = db_session.query(Score).filter(Score.record_id == rec.id).count()
    assert score_count == 1, f"Score should be preserved, got {score_count}"

    preserved = db_session.query(Score).filter(Score.id == old_score_id).first()
    assert preserved is not None
    assert preserved.total_score == 85
