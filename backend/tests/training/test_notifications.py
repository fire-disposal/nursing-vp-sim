...
def test_mark_all_read_requires_auth(client):
    resp = client.patch("/api/training/notifications/read-all")
    assert resp.status_code == 401


def test_mark_all_read_route_resolves(client, teacher, db_session):
    """Regression: /notifications/read-all must not be captured by /{notif_id}.
...
def test_mark_single_notification_read(client, teacher, db_session):
    ...
    resp = client.patch(
        f"/api/training/notifications/{notif.id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200
