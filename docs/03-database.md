...
## DEPRECATED 表 (保留用于迁移兼容，将在未来清理)

### api_providers — LLM API Provider 表 (已废弃)

被 `api_secrets` + `llm_configs` 替代。保留仅用于 Alembic 迁移兼容。
...
| api_type | VARCHAR(20) | openai_compatible |
| default_model | VARCHAR(80) | |
| is_enabled | BOOLEAN | |
| priority | INTEGER | DEFAULT 100 |
| created_at / updated_at | DATETIME | |

> **注意**: 旧版 `api_keys` 表已被完全移除。所有 API Key 管理整合到 `api_secrets` 表，模型配置整合到 `llm_configs` 表。

---

## 种子数据

服务启动时通过 lifespan 生命周期执行种子数据初始化：

1. **Alembic 迁移**: 自动执行 `alembic upgrade head` 创建最新表结构
2. **School 初始化**: 创建默认学校
3. **Role 初始化**: 创建系统角色 (admin, teacher, student) + RolePermission 记录
4. **User 初始化**: 检查 users 表是否为空，若为空则创建默认管理员 + 测试学生账号
5. **Grade/Class 初始化**: 创建默认年级和班级
6. **Case 初始化**: 从 `backend/data/cases/*.json` 读取病例数据并插入或更新 (Upsert)
7. **Rubric 初始化**: 从 `backend/data/rubrics/*.json` 加载评分标准
8. **ApiSecret 初始化**: 从 `.env` 读取 `DEEPSEEK_API_KEY` 创建默认 Secret + LLMConfig

---

## 数据迁移规范

- 使用 Alembic `--autogenerate` 自动生成 DDL 迁移
- DDL 迁移和数据迁移严格分开，不得混合在同一文件中
- DDL 迁移文件自动包含 `# ### commands auto generated by Alembic` 标记
- 数据迁移文件必须包含 `# Manual override reason: data_only` 文档字符串
