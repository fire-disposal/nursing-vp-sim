...
describe("QuestionnaireModal", () => {
  it("renders questions and title", () => {
    ...
    render(<QuestionnaireModal open={true} onComplete={vi.fn()} onSkip={vi.fn()} checkResponse={checkResponse} loading={false} onSubmit={vi.fn()} />);
  ...
  it("calls onSkip when skip button clicked", async () => {
    const onSkip = vi.fn();
    const checkResponse = makeCheckResponse({ is_required: false });
    render(<QuestionnaireModal open={true} onComplete={vi.fn()} onSkip={onSkip} checkResponse={checkResponse} loading={false} onSubmit={vi.fn()} />);
  ...
  it("shows validation error when required question is not answered", async () => {
    ...
    render(<QuestionnaireModal open={true} onComplete={vi.fn()} onSkip={vi.fn()} checkResponse={checkResponse} loading={false} onSubmit={onSubmit} />);
  ...
  it("calls onSubmit with answers when all required questions answered", async () => {
    ...
    expect(onSubmit).toHaveBeenCalledWith([{ question_id: 1, answer_value: "3" }]);
  });
  ...
  it("renders short text input", () => {
    ...
    expect(screen.getByPlaceholderText("请输入您的回答...")).toBeInTheDocument();
  });

  it("shows loading state", () => {
    const checkResponse = makeCheckResponse();
    render(<QuestionnaireModal open={true} onComplete={vi.fn()} onSkip={vi.fn()} checkResponse={checkResponse} loading={true} onSubmit={vi.fn()} />);
  ...
  it("calls onComplete after successful submit", async () => {
    ...
    render(<QuestionnaireModal open={true} onComplete={onComplete} onSkip={vi.fn()} checkResponse={checkResponse} loading={false} onSubmit={onSubmit} />);
