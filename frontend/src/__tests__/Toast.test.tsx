...
function ToastTrigger({ message = "test", type = "info" as const, duration = 0 }: { message?: string; type?: "success" | "error" | "warning" | "info"; duration?: number }) {
  const toast = useToast();
  return <button type="button" onClick={() => toast.toast(message, type, duration)}>Show</button>;
}
...
describe("Toast", () => {
  ...
  it("limits to max 5 toasts", async () => {
    function MultiToast() {
      const toast = useToast();
      return <button type="button" onClick={() => { for (let i = 0; i < 10; i++) toast.toast(`Toast ${i}`, "info", 0); }}>Flood</button>;
    }
