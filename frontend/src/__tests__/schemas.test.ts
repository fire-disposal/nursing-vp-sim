...
describe("loginSchema", () => {
  it("validates with valid data (username and password non-empty)", () => {
    const result = loginSchema.safeParse({ username: "testuser", password: "secret" });
  ...
  it("fails when username exceeds 50 chars", () => {
    const result = loginSchema.safeParse({ username: "a".repeat(51), password: "secret" });
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues).toContainEqual(expect.objectContaining({ path: ["username"] }));
    }
...
describe("changePasswordSchema", () => {
  ...
  it("fails when oldPassword is empty", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues).toContainEqual(expect.objectContaining({ path: ["oldPassword"] }));
    }
  });
  ...
  it("fails when newPassword < 6 chars", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues).toContainEqual(expect.objectContaining({ path: ["newPassword"] }));
    }
  });
  ...
  it("fails when confirmPassword doesn't match newPassword", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues).toContainEqual(expect.objectContaining({ path: ["confirmPassword"] }));
    }
  });
});
