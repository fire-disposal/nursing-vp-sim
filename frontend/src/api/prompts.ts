...
export const fetchPrompts = (purpose?: string) => {
	...
	return api.get<Schemas["PromptTemplateResponse"][]>("/admin/prompts", {
		params,
	});
};
...
export const deletePrompt = (id: number | string) =>
	api.delete<Schemas["OkResponse"]>(`/admin/prompts/${id}`);

export const activatePrompt = (id: number | string, purpose?: string) =>
	api.post<Schemas["PromptTemplateResponse"]>(
		`/admin/prompts/${id}/activate${purpose ? `?purpose=${encodeURIComponent(purpose)}` : ""}`,
	);
...
export const reloadPrompts = () =>
	api.post<Schemas["OkResponse"]>("/admin/prompts/reload");

export const previewActivePrompt = (purpose: string) =>
	api.get<Schemas["PromptPreviewResponse"]>("/admin/prompts/active/preview", {
		params: { purpose },
	});

export const fetchSampleVars = (purpose: string) =>
	api.get<Schemas["SampleVarsResponse"]>("/admin/prompts/sample-vars", {
		params: { purpose },
	});
