...
interface QuestionnaireModalProps {
  ...
  loading: boolean;
  onSubmit: (answers: { question_id: number; answer_value: string | null }[]) => Promise<void>;
}

const LIKERT_LABELS = ["非常不同意", "不同意", "一般", "同意", "非常同意"];

export function QuestionnaireModal({ open, onComplete, onSkip, checkResponse, loading, onSubmit }: QuestionnaireModalProps) {
  ...
  const handleAnswer = useCallback((questionId: number, value: string | null) => {
    ...
    <Modal open={open} onClose={() => {}} title={template.title} maxWidth={700}>
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {questions.map((q, idx) => (
              <div key={q.id} className="space-y-2 rounded-lg border border-border p-4">
                    ...
                    {[1, 2, 3, 4, 5].map((val) => (
                      <button
                        ...
                        className={`flex flex-col items-center gap-1 rounded-lg border px-3 py-2 text-xs transition-colors hover:bg-muted ${
                          answers[q.id] === String(val) ? "border-primary bg-primary/10 text-primary" : "border-border"
                        }`}
                      >
                        <span className="text-lg font-semibold">{val}</span>
                        <span className="text-[10px] text-muted-foreground">{LIKERT_LABELS[val - 1]}</span>
                      </button>
                    ))}
                  </div>
                )}
                    ...
                    {q.options.map((opt) => (
                      <label
                        ...
                        className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm cursor-pointer transition-colors hover:bg-muted ${
                          answers[q.id] === opt ? "border-primary bg-primary/10" : "border-border"
                        }`}
                      >
                        <input
                          type="radio"
                        ...
                        <span className="flex h-4 w-4 items-center justify-center rounded-full border border-border">
                          {answers[q.id] === opt && <span className="h-2 w-2 rounded-full bg-primary" />}
                        </span>
                        {opt}
                      </label>
                    ))}
