...
import { fetchPrompts, fetchSampleVars, validatePrompt } from "@/api/api-client";
...
import type { PromptForm as PromptFormType, PromptTemplateResponse, PromptValidateResponse, VariableMeta } from "./prompts/types";
import { PURPOSE_LABELS, PURPOSES } from "./prompts/types";
import { useActivatePrompt, useDeletePrompt, useSavePrompt } from "./prompts/usePromptMutations";
...
export default function PromptManagementTab() {
  ...
  const [form, setForm] = useState<PromptFormType>({ purpose: "patient_chat", name: "", system_prompt: "", user_prompt: "", remark: "", activate: true });
  ...
  const editedPrompt = editing != null && editing !== "new" ? (prompts.find((p) => p.id === editing) ?? null) : null;
  const isBuiltinEditing = editing === 0;

  const updateVarMeta = (field: "desc" | "default_value" | "source", varName: string, value: string) => {
    if (!editedPrompt) return;
    queryClient.setQueryData<PromptTemplateResponse[]>(queryKeys.prompts.list, (prev) =>
      (prev ?? []).map((p) => {
        if (p.id !== editedPrompt.id) return p;
        const updatedVars = ((p.variables || []) as unknown as VariableMeta[]).map((v) => (v.name === varName ? { ...v, [field]: value } : v));
  ...
  return (
    <div>
      <div className="mb-4 flex gap-2">
        <Button variant="outline" className="border-blue-600 text-blue-600" onClick={() => setShowActiveModal(true)}>
          <Eye size={13} /> 查看生效版本
        </Button>
      </div>
      ...
      <div className="grid grid-cols-[340px_1fr] gap-4 items-start min-h-[calc(100vh-180px)] max-[900px]:grid-cols-1">
        ...
        <PromptForm
          ...
          onUpdateVarDefault={(vName, defaultValue) => updateVarMeta("default_value", vName, defaultValue)}
          onUpdateVarSource={(vName, source) => updateVarMeta("source", vName, source)}
        />
      </div>
