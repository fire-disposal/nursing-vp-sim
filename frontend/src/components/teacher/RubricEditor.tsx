...
const inputClass = "border border-border rounded-md bg-card focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10";
      ...
      {dimensions.map((dim, di) => (
        <div key={di} className="border border-border rounded-lg p-3 bg-muted">
          <div className="flex gap-2 mb-2 items-center">
            <input
              className={cn(inputClass, "flex-1 px-2 py-1 text-sm font-semibold h-auto")}
              placeholder="维度名称"
              value={dim.name}
            ...
            <Button size="sm" variant="ghost" className="text-red-500 hover:bg-destructive/10" onClick={() => removeDim(di)} title="删除此维度">
              <Trash2 size={12} />
            </Button>
          </div>
          <input
          ...
          <div className="pl-2">
            {dim.items.map((item, ii) => (
              <div key={ii} className="mb-2 p-2 border border-border rounded-sm bg-card">
                <div className="flex gap-1.5 mb-1 items-center">
                  <span className="text-xs text-muted-foreground/70">{ii + 1}</span>
                  <input
                    className={cn(inputClass, "flex-1 py-0.5 px-1.5 text-xs h-7")}
                    placeholder="条目名称"
                    value={item.name}
                    onChange={(e) => updateItem(di, ii, { name: e.target.value })}
                  />
                  <Button size="sm" variant="ghost" className="text-red-500 hover:bg-destructive/10" onClick={() => removeItem(di, ii)} title="删除此条目">
                    <Trash2 size={11} />
                  </Button>
                </div>
                {["1", "2", "3"].map((score) => (
                  <div key={score} className="flex gap-1.5 mb-0.5 items-center">
                    <span className="text-[0.625rem] text-muted-foreground/70 w-7 text-right">{score}分</span>
                    <input
                      className={cn(inputClass, "flex-1 py-0.5 px-1.5 text-xs h-7")}
