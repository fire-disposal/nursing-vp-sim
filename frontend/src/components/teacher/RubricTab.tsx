...
export default function RubricTab() {
	...
	return (
		<div>
			...
			{rubrics.length === 0 && (
				<EmptyState icon={Layout} title="暂无评分标准" />
			)}
					...
					<div key={r.id} className="border border-border rounded-lg bg-card">
						...
						>
							<div>
								...
								<div className="text-xs text-muted-foreground mt-0.5">
									{r.description || "无描述"} · {dimCount(r)} · 满分
									{r.total_max}
								</div>
							</div>
						...
						{expandedId === r.id && (
							<div className="border-t border-border px-4 py-3 bg-muted rounded-b-lg">
								...
								{((r.dimensions as unknown as RubricDimension[]) || [])
									.length === 0 ? (
									<EmptyState
										icon={Layout}
										title="暂无评估维度"
									...
									((r.dimensions as unknown as RubricDimension[]) || []).map(
										(dim, i) => (
											<div key={i} className="mb-3">
												...
												<div className="pl-2">
													{(dim.items || []).map((item, j) => (
														<div
															key={j}
															className="mb-1 px-2 py-1 rounded-sm bg-card border border-border"
														>
															...
															{item.anchors && (
																<div className="text-xs text-muted-foreground pl-2">
																	{Object.entries(item.anchors).map(
																		([k, v]) => (
			...
			>
				<div className="flex flex-col gap-2.5 max-h-[70vh] overflow-auto">
					...
					<div>
						<div className="flex justify-between items-center mb-1.5">
							<label className="text-sm font-medium">
								...
								<span className="font-normal text-muted-foreground/70 text-xs ml-1.5">
									({formDims.length}维度 ·{" "}
									{formDims.reduce((s, d) => s + (d.items?.length || 0), 0)}
									条目)
								</span>
							</label>
						...
						{dimError && (
							<div className="text-destructive text-xs mt-1">{dimError}</div>
						)}
					</div>
				</div>
			</Modal>
