...
import type { BatchUser, EditUserFormValues, UserBrief, UserFormValues } from "./users/types";
import UserForm from "./users/UserForm";
import UserList from "./users/UserList";
import { useRolesQuery, useUserList } from "./users/useUserList";
import { useBatchCreateUsersMutation, useDeleteUserMutation, useRegisterMutation, useUpdateUserMutation } from "./users/useUserMutations";
...
export default function UsersTab({ currentUserId }: UsersTabProps) {
  ...
  const [classParam, setClassParam] = useState<{ grade_id: number | null; class_id: number | null } | null>(null);
  ...
  const handleSaveEdit = (form: EditUserFormValues) => {
    ...
    if (form.class_id !== undefined && form.class_id !== "") payload.class_id = Number(form.class_id);
  ...
  return (
    <>
      <div className="mb-4 flex gap-3">
        ...
        <button className={btnSecondary} onClick={() => setShowBatchImport(true)}>
          <Users size={16} /> 批量导入
        </button>
      </div>
