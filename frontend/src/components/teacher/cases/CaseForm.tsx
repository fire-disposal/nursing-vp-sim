...
import { buildCaseData, inputClass, NEW_CASE_TEMPLATE, parseCaseData, textareaClass } from "./types";
...
export default function CaseFormModal({ open, editingCase, startWithAiPanel, availableCases, onClose, onSaved }: CaseFormProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={editingCase ? `编辑病例: ${editingCase.name}` : "添加新病例"} maxWidth={800}>
      {caseMsg && (
        <div
          className={cn(
      ...
      <div className="mb-4">
        ...
        {showAiPanel && (
          <div className="mt-3 p-4 rounded-lg bg-purple-50/50 border border-purple-100">
            ...
            <Button onClick={() => handleAiGenerate(null)} disabled={aiGenerating || createMutation.isPending || updateMutation.isPending}>
              {aiGenerating ? (
                <>⟳ 生成中...</>
              ) : (
                <>
        ...
        <fieldset className="border border-border rounded-lg p-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              ...
              <textarea rows={3} value={caseForm.present_illness} onChange={(e) => updateField("present_illness", e.target.value)} className={textareaClass} />
            </div>
            <div>
