...
import { createCase, deleteCase, generateCase, updateCase } from "@/api/api-client";
...
export function useUpdateCase() {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: Parameters<typeof updateCase>[1] }) => updateCase(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.cases.managed.all });
    },
  });
...
export function useDeleteCaseConfirm() {
  ...
  const checkAndConfirm = async (c: CaseManageItem): Promise<boolean> => {
    ...
    return confirm({ title: "删除病例", message: `确定删除病例"${c.name}"吗？`, confirmLabel: "确定删除", danger: true });
  };
...
export function useGenerateCase() {
  return useMutation({
    mutationFn: (data: Parameters<typeof generateCase>[0]) => generateCase(data),
  });
}
