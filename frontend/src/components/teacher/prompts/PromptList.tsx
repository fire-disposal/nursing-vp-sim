		...
		<>
										...
										versions.map((v) => (
											...
											>
												...
												>
													{v.locked ? (
														<span
															className="text-xs text-muted-foreground/50 p-0.5"
															title="内置提示词不可删除"
														>
															🔒
														</span>
				...
				<select
					value={
						editing !== "new" && editing != null
							? prompts.find((p) => p.id === editing)?.purpose || ""
							: editing === "new"
								? formPurpose
								: ""
					}
			...
			{editing == null && (
				<div className="rounded-xl border border-border bg-card shadow-sm p-8 flex flex-col items-center justify-center min-h-[300px]">
					<Layers
						size={40}
						className="text-muted-foreground/70 opacity-50 mb-4"
					/>
					...
					<div className="text-sm text-muted-foreground/70 mb-4">
						点击版本名打开编辑器，或点击左侧{" "}
						<Plus size={12} className="inline align-middle text-primary" />{" "}
						为场景创建新版本
					</div>
					{!prompts.length && (
