...
import { activatePrompt, createPrompt, deletePrompt, updatePrompt } from "@/api/api-client";
...
export function useSavePrompt() {
  ...
  return useMutation({
    mutationFn: ({ editingId, form }: { editingId: number | "new"; form: PromptForm }) => {
      if (editingId !== "new") {
...
export function useActivatePrompt() {
  ...
  return useMutation({
    mutationFn: (p: PromptTemplateResponse) => activatePrompt(p.id, p.id === 0 ? p.purpose : undefined),
    onSuccess: (_data, p) => {
