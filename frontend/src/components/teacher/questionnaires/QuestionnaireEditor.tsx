...
import { emptyQuestion, inputClass, QUESTION_TYPE_OPTIONS, textareaClass } from "@/components/teacher/questionnaires/types";
...
export default function QuestionnaireEditor({ open, editingId, form, editMsg, isLoadingDetail, isSaving, onClose, onSave, setForm }: QuestionnaireEditorProps) {
  const addQuestion = () => {
    setForm((prev) => ({
      ...prev,
  ...
  const updateQuestion = (index: number, updates: Partial<(typeof form.questions)[number]>) => {
    setForm((prev) => ({
      ...prev,
  ...
  return (
    <Modal open={open} onClose={onClose} title={editingId ? "编辑问卷模板" : "新建问卷模板"} maxWidth={700}>
