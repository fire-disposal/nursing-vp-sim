import { BarChart3, ClipboardCheck, Edit3, FileText, Plus, Trash2 } from "lucide-react";
import type { TemplateListItem } from "@/components/teacher/questionnaires/types";
import { inputClass, TYPE_LABEL, TYPE_OPTIONS } from "@/components/teacher/questionnaires/types";
                  ...
                  <tr key={t.id} className="hover:bg-muted">
                    ...
                    <td className="px-4 py-3 border-b border-border">
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => onEdit(t)} title="编辑">
                          <Edit3 size={14} />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => onAssign(t)} title="分配病例">
                          <FileText size={14} />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => onViewStats(t)} title="查看数据">
                          <BarChart3 size={14} />
                        </Button>
                        <Button size="sm" variant="danger" onClick={() => onDelete(t)} title="删除">
                          <Trash2 size={14} />
                        </Button>
                      </div>
                    </td>
