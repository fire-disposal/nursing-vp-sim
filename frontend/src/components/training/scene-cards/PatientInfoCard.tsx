    ...
    <div className="space-y-4 p-3">
      <div className="flex items-center gap-3 pb-3 border-b">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{age ? `${age}岁` : ""} · {gender}</p>
        </div>
      </div>
      ...
      {healthLiteracy && (
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            <Tag label={healthLiteracy === "low" ? "健康素养低" : healthLiteracy === "high" ? "健康素养高" : "正常"} />
            <Tag label={verbosity === "terse" ? "寡言" : verbosity === "verbose" ? "健谈" : "正常交流"} />
            <Tag label={anxietyTrait === "anxious" ? "易焦虑" : "心态平稳"} />
          </div>
        </div>
      )}

      {features?.emotion && <LiveEmotion recordId={recordId} />}
    </div>
  );
}
  ...
  return (
    <div className="p-3 bg-blue-50 rounded-lg">
      ...
      <div className="space-y-1.5">
        <EmotionBar label="信赖" value={trust} color="bg-blue-500" />
        <EmotionBar label="舒适" value={comfort} color="bg-teal-500" />
      </div>
    </div>
  );
}
