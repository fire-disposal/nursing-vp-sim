          ...
          >
            ...
            <span
              style={{
                ...
                fontWeight: 700,
                color:
                  pct >= 80 ? "#15803d" : pct >= 40 ? "#b45309" : "#dc2626",
              }}
            >
              {covered}/{total}
            </span>
            ...
            return (
              ...
              >
                {done ? (
                  ...
                  <Circle
                    size={16}
                    style={{ color: "#d1d5db", flexShrink: 0, marginTop: 1 }}
                  />
                )}
      ...
      .then(({ data }) => {
        ...
        const r =
          detail.remaining_seconds != null
            ...
            : Math.max(
                0,
                (detail.time_limit || 20) * 60 -
                  Math.floor(
                    (Date.now() - new Date(detail.start_time).getTime()) / 1000,
                  ),
              );
        setRemaining(r);
        setTimerActive(true);
    ...
    await sendMessageStream(
      ...
      (doneId?: number) => {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === patientMsgId
              ? { ...msg, streaming: false, id: doneId || msg.id }
              : msg,
          ),
        );
    ...
    try {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        if (detail.data.scoring_status === "failed") {
          toast.error(
            `自动评分失败：${detail.data.scoring_error || "未知错误，可在训练记录中手动重试"}`,
          );
          break;
        }
      }
      ...
      if (axiosErr.name !== "CanceledError" && axiosErr.code !== "ERR_CANCELED") {
        if (!isAuto)
          toast.error(axiosErr.response?.data?.detail || "结束训练失败，请重试");
      }
    } finally {
      setEnding(false);
    ...
    <div className="training-shell">
      <header className="training-topbar">
        <button
          ...
          onClick={async () => {
            const isActive =
              remaining != null && remaining > 0 && !score && !ending;
            if (isActive) {
              const ok = await confirm({
                title: "离开训练",
        ...
        <div className="training-patient-identity">
          ...
          />
          <div>
            <div className="training-patient-name">
              {patientName || "虚拟患者"}
            </div>
      ...
      <div className="training-input-bar">
        ...
        <input
          ...
          placeholder={
            remaining === 0 ? "训练时间已结束" : "输入你的问题，按 Enter 发送..."
          }
          disabled={loading || ending || remaining === 0}
        />
        <button
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: 3,
                  background:
                    scoreProgress >= 100 ? "#22c55e" : "var(--blue-600)",
                  width: `${scoreProgress}%`,
                  transition:
                    scoreProgress >= 100
                      ? "none"
