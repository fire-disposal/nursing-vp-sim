          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
            ...
            <span
              style={{
                ...
                fontWeight: 700,
                color:
                  item.score >= 3 ? "#15803d" : item.score >= 2 ? "#b45309" : "#dc2626",
              }}
            >
              {item.score}/3
            </span>
          ...
          {[1, 2, 3].map((s) => (
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border:
                  currentScore === s ? "2px solid #2563eb" : "1px solid #d1d5db",
...
function ReviewEditor({
  ...
  const handleSubmit = () => {
    const modified = JSON.parse(
      JSON.stringify(detailScores),
    ) as Record<string, DetailScoreCategory>;
...
export default function RecordDetail() {
  ...
  const scoreMax = record.score?.detail_scores
    ? Object.values(record.score.detail_scores).reduce(
        (sum: number, value: unknown) => {
          if (value && typeof value === "object" && "max" in (value as Record<string, unknown>))
            return sum + ((value as Record<string, number>).max || 0);
          return sum + 30;
        },
        0,
      )
            ...
            >
              <span
                style={{
                  fontWeight: 600,
                  color:
                    msg.role === "student" ? "var(--primary)" : "#0d9488",
                  marginRight: 8,
                }}
              >
