    ...
    <>
      ...
      {trends && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-6">
          <StatCard icon={Activity} value={trends.total_sessions} label="总训练次数" color="blue" />
          <StatCard icon={Clock} value={trends.total_minutes} label="总训练时长（分钟）" color="amber" />
          <StatCard icon={Target} value={trends.avg_score != null ? `${trends.avg_score}分` : "-"} label="平均得分" color="green" />
          <StatCard
            icon={TrendingUp}
            value={trends.total_sessions > 0 ? `${Math.round(trends.total_minutes / trends.total_sessions)}分钟` : "-"}
            label="平均每次训练时长"
            color="teal"
          />
        </div>
      ...
      <Card className="mb-6">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold">训练投入：次数与时长</CardTitle>
        </CardHeader>
        <CardContent>
          {hasData ? (
      ...
      <Card className="mb-6">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-semibold">训练效果：次数与得分</CardTitle>
        </CardHeader>
        <CardContent>
          {hasData ? (
            <ResponsiveContainer width="100%" height={280}>
              ...
              <TableBody>
                {summary.map((s) => (
                  <TableRow key={s.user_id}>
                    ...
                    <TableCell className="font-semibold">{s.total_minutes}</TableCell>
        ...
        <Card>
          ...
          <div className="max-h-96 overflow-auto">
            <Table>
              ...
              <TableBody>
                {ranking.map((s) => (
                  <TableRow key={s.user_id} className={cn(s.rank <= 3 && "bg-amber-50")}>
                    <TableCell>
                      {s.rank === 1 ? (
                    ...
                    <TableCell>{s.total_sessions}</TableCell>
                    <TableCell className={cn("font-bold", s.avg_score != null ? "text-primary" : "text-gray-400")}>
                      {s.avg_score != null ? `${s.avg_score}分` : "-"}
                    </TableCell>
                    <TableCell className="text-muted-foreground">{s.total_score > 0 ? `${s.total_score}分` : "-"}</TableCell>
                    <TableCell className="text-muted-foreground">{s.total_minutes}分钟</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
function EmptyChart() {
  return <EmptyState icon={BarChart3} title="暂无该时间段的数据" className="min-h-[200px]" />;
}
