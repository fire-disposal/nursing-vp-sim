...
import { CTA_HREF, CTA_LABEL, HERO_SUBTITLE, PRODUCT_NAME } from "../data";
...
export default function Hero() {
	...
	return (
		<section
			ref={root}
			className="relative flex min-h-[calc(100dvh-64px)] items-center justify-center overflow-hidden py-16"
		>
			...
			<div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-14 px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-20">
				<div className="flex flex-col gap-8 lg:pt-10">
					...
					<p
						ref={subtitleRef}
						className="max-w-[50ch] text-lg leading-relaxed text-muted-foreground md:text-xl lg:text-[1.65rem]"
					>
						{HERO_SUBTITLE}
					</p>
					...
					<div ref={ctaRef} className="flex flex-wrap items-center gap-5">
						...
						<button className="group flex items-center gap-3 text-sm font-semibold transition-colors hover:text-primary">
				...
				<div ref={panel} className="relative lg:pt-8">
					...
					<div className="relative overflow-hidden rounded-[2rem] border border-border/60 bg-card/80 p-4 shadow-[0_24px_70px_-40px_rgba(15,23,42,0.45)] backdrop-blur-sm md:p-6">
