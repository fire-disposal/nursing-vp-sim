...
function drawQuadrantLabels(ctx: CanvasRenderingContext2D, w: number, h: number) {
  ctx.fillStyle = "rgba(128,128,128,0.18)";
  ctx.font = "11px sans-serif";
  for (const { x, y, text } of Object.values(QUADRANT_LABELS)) {
    ctx.fillText(text, PADDING + (w - PADDING * 2) * x, PADDING + (h - PADDING * 2) * y);
  }
}
...
function drawPoint(ctx: CanvasRenderingContext2D, trust: number, comfort: number, w: number, h: number, alpha: number, r: number, glow: boolean) {
...
export function EmotionTrajectory({ history, current }: EmotionTrajectoryProps) {
  ...
  useEffect(() => {
    ...
    if (history && history.length > 0) {
      for (let i = 0; i < history.length; i++) {
        ...
        drawPoint(ctx, history[i].trust, history[i].comfort, w, h, alpha, r, false);
      }
    }
  ...
  return (
    <div className="rounded-lg border bg-muted/20 overflow-hidden" style={{ aspectRatio: "1" }}>
