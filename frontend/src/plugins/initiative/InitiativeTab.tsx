...
export function InitiativeTab({ ctx }: PanelTabProps) {
	...
	useEffect(() => {
		const unsub1 = ctx.bus.on("initiative:state", (s: InitiativeState) =>
			setState(s),
		);
		const unsub2 = ctx.bus.on("initiative:triggered", () =>
			setHasPending(true),
		);
				...
				<div className="flex justify-between text-xs text-muted-foreground">
					...
					<span className="tabular-nums">
						{state.elapsed_seconds != null
							? `${Math.round(state.elapsed_seconds)}s / ${Math.round(state.threshold_seconds || 60)}s`
							: "计算中..."}
					</span>
				</div>
