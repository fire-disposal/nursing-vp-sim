...
export default function VitalSignItemField({
	item: _item,
	value,
	onChange,
}: VitalSignItemProps) {
		...
		<div className="grid grid-cols-3 gap-x-3 gap-y-2">
			{FIELDS.map((field) => (
				<div key={field.key} className="flex flex-col gap-0.5">
					<label className="text-[0.6rem] font-medium text-muted-foreground">
						{field.label}
					</label>
					<div className="relative">
						...
						/>
						<span className="absolute right-2 top-1/2 -translate-y-1/2 text-[0.6rem] text-muted-foreground pointer-events-none">
							{field.unit}
						</span>
					</div>
				</div>
