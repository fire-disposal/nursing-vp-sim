...
import type { MessageBus, ScoreData, ScoreDimension, ScoreDimensionItem } from "@/engine/types";
...
function DetailItem({ name, item }: { name: string; item: ScoreDimensionItem }) {
  return (
        ...
        {score.detail_scores && (
          <div className="mb-4 space-y-2">
            ...
            {Object.entries(score.detail_scores).map(([dimName, dim]) => {
              ...
              return (
                <div key={dimName} className="rounded border border-border bg-card p-2">
...
export function ScoreCard({ bus, recordId }: { bus: MessageBus; recordId: string }) {
