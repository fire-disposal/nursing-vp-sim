...
const STAGES = [
	...
	},
	{
		label: "发布审批",
		accent: "pink",
		desc: "打 Tag 触发部署流水线，并绑定 GitHub Environment 审批：必须由人工放行才会真正发版（单实例部署，不再有独立的测试服）。每次部署留痕，可精确回退到历史版本。",
		chips: [
			{ k: "自动触发", v: "Tag push" },
			{ k: "人工闸门", v: "环境审批" },
			{ k: "版本留痕", v: "部署历史" },
	...
	},
	{
		label: "生产发布",
		accent: "green",
		desc: "审批通过后部署至 iomt.205716.xyz：部署前自动备份数据库（失败即停），部署后健康检查与诊断端点冒烟，异常秒级回滚，全程零停机。",
		chips: [
			{ k: "备份门禁", v: "失败即停" },
			{ k: "镜像校验", v: "存在性检查" },
			{ k: "秒级回滚", v: "失败自恢复" },
